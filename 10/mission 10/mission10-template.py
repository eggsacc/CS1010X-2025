...
# Time complexity of insert_tree: O(log(n)), worst case O(n) if list is completely skewed and only has left/right branches
...
# Time complexity of contains: O(log(n)), worst case O(n) if list is skewed and value is at lowest depth
...
def flatten(tree):
    ...
    # Your code here
    # Left, right both empty: return entry
    if(is_empty_tree(left_branch(tree)) and  is_empty_tree(right_branch(tree))):
        return [entry(tree)]
    # Only left empty: return entry + flattened right
    elif(is_empty_tree(left_branch(tree))):
        return [entry(tree)] + flatten(right_branch(tree))
    # Only right empty: return flattened left + entry
    elif(is_empty_tree(right_branch(tree))):
        return flatten(left_branch(tree)) + [entry(tree)]
    # Both branches present: flatten left + entry + flatten right
    else:
...
# Time complexity of flatten: O(n)
...
def sort_it(lst):
    # Your code here
    sorted_tree = accumulate(lambda x, y: insert_tree(x, y), make_empty_tree(), lst)
...
# Time complexity of sort_it: O(n + n * log(n)) => Simplifies to O(n * log(n)) since nlog(n) dominates as n -> inf
#                             Worst case: O(n^2) assuming worst case for all [n] insert_tree() calls
